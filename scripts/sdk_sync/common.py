...
def parse_env_file(path: Path) -> dict[str, str]:
    """Parse simple KEY=VALUE lines (blank lines and '#' comments ignored)."""
    ...
    for raw_line in path.read_text(encoding="utf-8").splitlines():
        ...
        if not match:
            sys.exit(f"Invalid line in {path}: {raw_line!r}")
...
def git_sign_off_line() -> str:
    return f"Signed-off-by: {GIT_USER_NAME} <{GIT_USER_EMAIL}>"
