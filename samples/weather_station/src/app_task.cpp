...
void AppTask::UpdateTemperatureClusterState()
{
	...
	if (result == 0) {
		...
		CHIP_ERROR err =
			Clusters::TemperatureMeasurement::SetMeasuredValue(kTemperatureMeasurementEndpointId, newValue);
...
void AppTask::UpdatePressureClusterState()
{
	...
	if (result == 0) {
		...
		CHIP_ERROR err =
			Clusters::PressureMeasurement::SetMeasuredValue(kPressureMeasurementEndpointId, newValue);
		if (err != CHIP_NO_ERROR) {
...
void AppTask::UpdateRelativeHumidityClusterState()
{
	...
	if (result == 0) {
		...
		CHIP_ERROR err = Clusters::RelativeHumidityMeasurement::SetMeasuredValue(kHumidityMeasurementEndpointId,
											 newValue);
		if (err != CHIP_NO_ERROR) {
